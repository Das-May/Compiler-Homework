...
public:
     ...
     * @return DFA Table is also a 2D Table represented by a 1D array,\
     *  every table cell is a vector<int>, and the DFA[0][0] is the number of row, \
     *  DFA[0][1] is the col, other element of DFA[0] are the ID numbers of end nodes of DFA
     */
    static vector<vector<int>> GetDFA(const vector<vector<int>>& NFA);

    /**
     * @brief Generate minimized DFA Table by process DFA Table
     * @param DFA is a 2D Table represented by a 1D array, \
     * every table cell is a vector<int>, and the DFA[0][0] is the number of row, \
     * DFA[0][1] is the col, other element of DFA[0] are the ID numbers of end nodes of DFA
