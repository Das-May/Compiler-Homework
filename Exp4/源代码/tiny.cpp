...
BTreeNode* Tiny::stmt_sequence()
{
    ...
    nodex = statement();                // 第1个元素是根节点

    while (token.ID == SEMICOLON)       // 【右递归】若存在第2个元素
    {
        nodey = new BTreeNode(token);   // 取第2个元素生成新的根结点
        nodey->lc = nodex;              // 左子结点是第1个元素
        match(token.ID);
        nodey->rc = statement();        // 右子节点是第3个元素
        nodex = nodey;                  // 更新根节点
    }
    return nodex;
}
