...
int main(){
...
#ifdef__APPLE__glfwWindowHint (GLFW_OPENGL_FORWARD_COMPAT,GL_TRUE);
	
#endifGLFWwindow*window=glfwCreateWindow(SCR_WIDTH,SCR_HEIGHT,"LearnOpenGL",NULL,NULL);
	if (window==NULL){
		std::cout<<"FailedtocreateGLFWwindow"<<std::endl;
		glfwTerminate();
		return -1;
		}
	...
	ShaderourShader("6.3.coordinate_systems.vs","6.3.coordinate_systems.fs");
	float vertices[]={
	...
	if (data){
		...
		std::cout<<"Failedtoloadtexture"<<std::endl;
		}
	...
	while (!glfwWindowShouldClose(window)){
		...
		glBindTexture(GL_TEXTURE_2D,texture2);
		ourShader.use();
		glm::mat4view=glm::mat4(1.0f);
		glm::mat4projection=glm::mat4(1.0f);
...
fer_size_callback(GLFWwindow*window,int width,int height){
